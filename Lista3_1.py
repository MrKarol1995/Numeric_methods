...
def main():
    print("Rozwiązanie Eliminacja Gaussa: ")
...
def main():
    print("Rozwiązanie z biblioteki : ")
    x = linalg.solve(A, b)
    print(x)
...
def plot_eta_values(eta_values):
    ...
    Dodaje linię przerywaną na maksymalnej wartości eta_k.

    Parametry:
    - eta_values: lista wartości eta_k dla każdego k
    """
    plt.plot(eta_values, marker='o', label=r'$\eta_k$')
    plt.xlabel('Iteracja k')
    plt.ylabel(r'$\eta_k = \frac{||x^{(k)}||_2}{||x^{(0)}||_2}$')
    plt.title('Wartości $\eta_k$ w zależności od liczby iteracji')

    # Znajdź maksymalną wartość eta_k i jej indeks
    max_eta = max(eta_values)
    max_index = eta_values.index(max_eta)

    # Dodaj linię przerywaną na poziomie maksymalnej wartości eta_k
    plt.axhline(y=max_eta, color='r', linestyle='--', label=f'Max η, dla k = {max_index}')
